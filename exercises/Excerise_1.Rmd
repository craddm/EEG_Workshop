...
```{r gather_to_long}
gather(ObjFreqERPs, condition, amplitude, -Subject, -Time)
```
...
final_df <- ObjFreqERPs %>%
  ...
  separate(condition, into = c("Object", "Spatial Frequency"), sep = "/")
head(final_df)
```
...
```{r facet_plot}
ggplot(final_df, aes(x = Time, y = amplitude, colour = `Spatial Frequency`, fill = `Spatial Frequency`)) +stat_summary(fun.y = mean, geom = "line", size = 1) + facet_wrap("Object")
```
