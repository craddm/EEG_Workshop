...
First, load the packages *tidyverse* and *afex*, and reload the data frame you saved from the first exercise using load() or read_csv().
...
final_df %>%
  filter(Time >= 85 & Time <= 135) %>%
  group_by(Object) %>%
  summarise(stdev = sd(amplitude),  se = stdev/n(), amplitude = mean(amplitude))
```
