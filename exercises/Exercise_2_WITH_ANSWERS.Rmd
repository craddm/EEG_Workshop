...
Use filter to select rows where Time >= 85 and Time <= 135. Note that with multiple conditions, you must still specify 

*group_by()* is used to group rows together. Specify which variables you want.
...
Before running statistics, let's do some more plots.
...
We can do much better than this. Let's plot individual data points using geom_point() and join those points with a line so that we can see which points go together.
...
This looks like Object amplitudes are overall higher, although the distributions substantially overlap. The direction is fairly consistent across subjects, with only one person going in the opposite direction.

Let's go ahead with our paired t-test.
...
aov_ez( "Subject", "amplitude", hmm, within = c("Object", "`Spatial Frequency`"))
